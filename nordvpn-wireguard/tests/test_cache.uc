...
// Fixture: 4 servers — 3 WireGuard (1 multihop de-nl), 1 non-WireGuard (skipped).
check('countries', out.stats.countries, 3);
check('cities', out.stats.cities, 3);
check('gateways', out.stats.gateways, 3);
check('servers_seen', out.stats.servers_seen, 4);

let multihop = 0, single = 0;
for (let c in out.countries)
	for (let ci in c.cities)
		for (let r in ci.relays)
			r.multihop ? multihop++ : single++;
